...
async def test_chain_is_off_by_default(live_settings: Settings, person: SearchSubject) -> None:
    settings = live_settings.model_copy(
        update={"newdb_api_key": "k", "newdb_base_url": BASE_URL}
    )
...
async def test_no_companies_is_not_the_same_as_fns_silent(
    ...
    silent = await provider.fetch(
        ...
        FetchContext(
            upstream=(
                ProviderResult(provider=ProviderName.FNS, status=ProviderStatus.UNAVAILABLE),
            )
        ),
    )
...
async def test_cap_is_reported_not_silent(
    ...
    companies = [
        company(inn=f"972801282{digit}", name=f"ООО {digit}", bankrupt=False)
...
@respx.mock
async def test_bankrupt_companies_are_checked_first(
    provider: NewDBLegalCasesProvider, person: SearchSubject
) -> None:
    """Колпак маленький, поэтому порядок решает, что именно за него заплатят."""
    route = respx.post(NEWDB_URL).mock(
        return_value=httpx.Response(200, json=live_fixture("arbitr_legal.json"))
    )
    companies = [
        company(inn="1000000001", name="ООО Тихое", bankrupt=False),
        company(inn="1000000002", name="ООО Тихое-2", bankrupt=False),
        company(inn="1000000003", name="ООО Тихое-3", bankrupt=False),
        company(inn="1000000004", name="ООО Банкрот", bankrupt=True),
    ]

    await provider.fetch(person, FetchContext(upstream=(fns_result(*companies),)))

    queried = [json.loads(call.request.content)["params"]["inn"] for call in route.calls]
    assert "1000000004" in queried
