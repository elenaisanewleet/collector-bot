...
async def test_going_to_the_menu_keeps_the_card(
    ...
    assert sent.contains("✎ Фамилия"), "на ненайденный номер не задан вопрос"
...
async def test_a_number_that_missed_asks_for_a_surname(
    dispatcher: Dispatcher, bot: Bot, sent: SentMessages
) -> None:
    """Прислали номер, а в выгрузке его нет — бот спрашивает фамилию.

    Требование менялось дважды, и это надо знать, прежде чем «вернуть как
    было». Сначала на такой номер приходил каталог из восьми «+ поле — что оно
    откроет», дублировавший словами кнопки под тем же сообщением. Его убрали, и
    вместо него стала приходить карточка сбора — форма на семнадцать строк.

    Владелица закрыла и это, 08.09.2026, дословно: «у нас логика же, если
    телефон не сработал, пишем не найдено в базе, введите фамилию»,
    «просто фамилию точнее». Ответ известен заранее и короток, а форма на
    семнадцать строк вместо него — это ровно то «полотно», от которого
    избавлялись.

    Каталог опций при этом не должен вернуться ни в каком виде — за этим тест
    следит по-прежнему.
    """
    await feed(dispatcher, bot, message=make_message("89990000001"))

    card = sent.joined
    assert "никого не нашёл" in card, "бот не сказал, что не нашёл"
    assert "✎ Фамилия" in card, "бот не спросил фамилию"
    assert "Проверка должника" not in card, "вместо вопроса приехала форма"
    assert "откроет ещё" not in card, "каталог опций вернулся в форму"
    assert "+ Дата рождения" not in card
    assert "+ Паспорт" not in card
    # Оговорки про смысл прочерка здесь больше нет и быть не должно: прочерков
    # на этом экране нет — экран задаёт один вопрос. Сама оговорка никуда не
    # делась, она на карточке, там где прочерки и стоят.
...
async def test_an_exact_key_in_a_free_line_finds_the_debtor(
    ...
    Дословное требование владелицы: «ввёл номер — сразу отчёт», а фамилию бот
    просит только если по номеру никого не нашёл. Ориентир интерфейса говорит о
    том же: «никакого экрана "введите телефон", оператор просто пишет номер в
    чат, мгновенный ответ».

    Прежде выгрузка спрашивалась только в ведомом сценарии по кнопкам, и
    оператор, написавший номер, получал пустую форму. Потом карточка стала
    опознавать, но ждала нажатия «Проверить» — то есть вопроса ни о чём: точный
    ключ уже дал ровно одного человека.
    ...
    chat = sent.joined
    # Что выгрузка поднята, видно по её содержимому, а не по объявлению о
    # находке: имя и сумма долга взялись из строки, найденной по госномеру.
    # Самого объявления больше нет — карточка при автопрогоне не рисуется, а
    # отдельной строкой «нашёл того-то» перед отчётом был бы пересказ отчёта.
    assert "Тестов Андрей Сергеевич" in chat, "точный ключ не поднял строку выгрузки"
    assert "38 400 ₽" in chat, "долг из выгрузки не доехал"
    assert "RECOVERY SCORE" in chat, "точный ключ не довёл до отчёта"
    assert "откроет ещё" not in chat, "каталог опций вернулся в форму"
    assert "1С" not in chat
...
async def test_a_cold_card_starts_a_new_person(
    ...
    assert "✎ Номер или ФИО" in screen, "не спросил номер заново"
