...
async def test_reading_buttons_do_not_abandon_a_half_finished_search(
    ...
    # Введённое не потерялось: бот пошёл проверять именно этого человека, а не
    # ответил «начните сначала». Карточки в ответе нет и не должно быть — по
    # полному ФИО в выгрузке нашёлся ровно один, и проверка идёт сразу.
    assert sent.contains("Тестов Андрей Сергеевич")
    # Сумма, а не подпись формы: без публичного адреса отчёт уходит текстом, и
    # «Наш долг» там называется «Задолженность». Сама сумма — из выгрузки в
    # обоих случаях.
