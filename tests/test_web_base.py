...
def test_storage_says_last_not_only() -> None:
    ...
    assert "Всего задержаний" in html


# ------------------------------------------------------------------- тема


def test_the_unchecked_hatching_survives_the_dark_theme() -> None:
    """Штриховка «не проверено» обязана быть видна в обеих темах.

    Она рисуется поверх поверхности, а не поверх --warn-bg, и была задана
    тёмно-янтарным на 13%: на белом читается, на #161C25 исчезает полностью.
    То есть в тёмной теме единственный признак «источник молчал» был невидим —
    а весь продукт держится на том, что «не спрашивали» и «не нашли» выглядят
    по-разному. Это уже стояло на проде.
    """
    from app.web.style import CSS

    dark = CSS.split("prefers-color-scheme:dark", 1)[1]
    assert "--hatch:" in dark, "у тёмной темы нет своей штриховки"

    # И ни одного цвета штриховки мимо токена: следующая такая строка снова
    # окажется невидимой ровно в одной из тем, и заметят это не сразу.
    for block in CSS.split("repeating-linear-gradient")[1:]:
        head = block.split("}", 1)[0]
        if "rgba(122,88,9" in head:
            raise AssertionError(f"штриховка мимо --hatch: {head.strip()}")


def test_a_filter_that_filters_nothing_is_not_drawn() -> None:
    """Полоса вкладок появляется, только когда есть что делить.

    На выгрузке заказчика все две тысячи попадают в «судебный приказ» — долги
    от 5 000 до 230 000 ₽, — и полоса выходила из двух кнопок, означающих одно
    и то же: «Все 2052» и «Судебный приказ 2052». Фильтр, который ничего не
    отфильтровывает, читается как сломанный.
    """
    assert 'class="tabs"' not in page(["11970", "38400"])
    assert 'class="tabs"' in page(["11970", "640000"])


def test_the_summary_says_what_share_the_fees_eat() -> None:
    """Доля пошлины в требованиях — число, ради которого сводку и читают.

    На живой выгрузке типичный долг это одна эвакуация с парой суток хранения,
    а пошлина по нему одна и та же при любом таком долге. Вместе они забирают
    около трети всей суммы, и без этой строки заказчик видит два больших числа
    и не соотносит их.
    """
    # Долг 100 000 ₽ — пошлина по приказу 2 000 ₽, ровно 2%.
    assert "2% от требований" in page(["100000"])


def test_the_person_page_shows_every_field_of_the_table() -> None:
    """Страница заводилась затем, чтобы не лезть в 1С, — значит показывает всё.

    Раньше здесь была выборка из восьми полей, и заказчик, привыкший смотреть в
    1С, недосчитывался остального: VIN, заявки, дат постановки и выдачи по
    отдельности, номера записи в учёте. Дословная просьба владелицы: «переходя
    на ФИО отображать все поля из таблицы, которые есть».
    """
    html = person("38400")
    ...
    for field in (
        ...
        assert field in html, f"поля «{field}» нет на странице"


def test_empty_fields_are_shown_as_a_dash_not_hidden() -> None:
    """Прочерк здесь значит «в выгрузке этого нет» — и это полезный факт.

    По нему видно, какой колонки не хватает, чтобы бот заработал полнее.
    Скрытая строка выглядела бы так, будто поля не существует вовсе.
    """
    html = person("38400")

    # У тестового должника нет ни ИНН, ни VIN, ни договора.
    assert "—" in html
    assert "ИНН" in html and "VIN" in html
