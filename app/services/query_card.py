...
#: и всё, остальное это опция», — и у телефона первое место не по вкусу: это
#: единственный идентификатор, который бот умеет РАЗВЕРНУТЬ. В выгрузке 1С
#: телефон есть почти всегда, и по нему поднимается строка с ФИО, датой
#: рождения, договором и машиной. Спросить его первым — значит в типичном
#: случае не задавать остальных вопросов вовсе.
#:
...
class QueryCardService:
    ...
    def apply(self, card: Card, raw: str) -> Applied:
        ...
        if expected is Field.PHONE and card.step is Field.PHONE:
            # Первый шаг принимает ЛЮБОЙ номер, а не только телефон, и это не
            # послабление, а условие работоспособности. В выгрузке заказчика
            # телефона нет ни у одного из 2052 должников — и не будет, это часть
            # таблицы 1С. Госномер есть у 2008. Шаг, который принимает только
            # телефон, промахивается всегда, а «ввёл номер — сразу отчёт»
            # становится обещанием, которого бот не держит.
            #
            # Форма записи решает сама: госномер ляжет в госномер, ИНН в ИНН,
            # телефон в телефон. Гадания тут нет — гадание только у одинокого
            # слова буквами, и оно показывается вслух.
