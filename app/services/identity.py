...
class IdentityMatcher:
    ...
    def assess(self, subject: SearchSubject, record: SourcedFact) -> MatchAssessment:
        if isinstance(record, (AccountBlockRecord, TaxDebtRecord, SelfEmployedRecord)):
            ...
            # записям ноль, то есть выбросил бы из отчёта два источника, каждый
            # из которых отвечает на свой вопрос единственным доступным способом.
            #
            # Основание здесь — не содержание записи, а СПОСОБ ЗАПРОСА, ровно как
            # у связи с юрлицом ниже. Оба метода принимают только
            # двенадцатизначный ИНН физлица и отвечают о нём одном; отправить
            # туда ФИО нельзя — поставщик отвергнет запрос. То есть ответ
            # относится к субъекту по построению, и никакого допущения с нашей
            # стороны в этом нет.
            return MatchAssessment(
                confidence=IDENTIFIER_LOOKUP_CONFIDENCE,
                reasons=("найдено по ИНН должника",),
            )
