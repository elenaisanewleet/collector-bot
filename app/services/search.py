...
class SearchService:
    ...
    async def _resolve_inn(
        ...
        ждут её вместе со всеми.

        Цена фазы записана здесь, а не спрятана, и она ВЫШЕ, чем тут стояло
        раньше. Прежняя оценка — «плюс ``request_timeout_seconds * 2``,
        тридцать секунд» — считала мост обычным HTTP-запросом. Он не обычный:
        ``passport_fns`` асинхронный, и по журналу самого поставщика его медиана
        79 с при максимуме 207 с. То есть фаза стоит восемьдесят секунд типично,
        а сверху ограничена только ``provider_budget_seconds``.

        Развязать её по зависимостям — пустить мост одновременно с ФССП и
        залогами, а ИНН-зависимые источники стартовать по готовности моста —
        посчитано и отвергнуто. По тем же медианам (мост 79, ФССП 108, залоги
        67, банкротство 71, ЕГРИП 101, арбитраж 31) сейчас проверка занимает
        79 + 108 = 187 с, а с развязкой ``max(108, 79 + 101)`` = 180 с. Шесть
        секунд: ИНН-зависимые всё равно ждут мост, а ЕГРИП почти так же
        медленный, как самый медленный независимый. Правка ядра поиска ради 3%
        времени не окупается — а вот ``provider_concurrency`` окупается сразу:
        с единицы на пятёрку та же проверка идёт 187 с вместо 458 с.
