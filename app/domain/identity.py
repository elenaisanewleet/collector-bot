...
def is_name_word(word: str | None) -> bool:
    ...
    одному слову, и ей нужно отличить «Иванова» от «77091», не ослабляя сам
    разбор ФИО и не заводя вторую копию алфавита.
    """
    return bool(word and _NAME_ALLOWED.match(word))


#: Кириллица в латиницу — для СРАВНЕНИЯ имён, а не для показа.
#:
#: Нужно потому, что один и тот же человек приходит от поставщика тремя
#: записями: «Клочкова klochkova Елена elena Николаевна» (кириллица и транслит
#: вперемешку в одном поле), «Klochkova Elena» и «Клочкова Елена Николаевна».
#: Сравнение строк здесь бессильно, а решает оно немало: по совпадению имени
#: блок получает право говорить за должника — в том числе называть его адрес.
_TO_LATIN = {
    "а": "a",
    "б": "b",
    "в": "v",
...
#: Разные школы транслитерации об одном звуке. Приводятся к одному виду ПОСЛЕ
#: перевода, чтобы «Klochkova» из ответа и «Клочкова» из выгрузки сошлись, как
#: сошлись бы «Shchukin», «Schukin» и «Щукин».
_LATIN_EQUIVALENTS = (
    ("shch", "sch"),
    ("kh", "h"),
    ("yu", "iu"),
...
def translit_ru(word: str) -> str:
    """Слово, приведённое к сравнимому виду: латиница, нижний регистр.

    Только для СВЕРКИ. Показывать результат человеку нельзя: «Клочкова»
    превращается в «klochkova», и в карточке это выглядело бы как ошибка.

    Перевод приблизительный намеренно. Точная транслитерация по ГОСТу здесь не
    нужна и даже вредна: задача — узнать одно имя в разных записях, а не
    воспроизвести его. Поэтому «х» и «kh», «ю» и «yu», «щ» и «shch» считаются
    одним и тем же.
    """
    lowered = "".join(
        _TO_LATIN.get(letter, letter) for letter in word.strip().lower() if letter.isalnum()
    )
    for source, target in _LATIN_EQUIVALENTS:
...
class SearchSubject(BaseModel):
    ...
    @property
    def known_alive_at(self) -> date | None:
        """Самая поздняя дата, на которую должник ЗАВЕДОМО был жив.

        Нужна одному различителю, но важному: реестр наследственных дел ищет по
        одному ФИО и возвращает всех однофамильцев, а даты рождения в его
        записях чаще всего нет вовсе. Дата смерти есть всегда — и запись о
        смерти, случившейся раньше, чем должник заведомо был жив, это запись о
        другом человеке. Никакое совпадение ФИО такого не перевешивает.

        Рождения для этого мало. Живой случай: смерть 27.04.1996, должник
        родился 24.11.1994 — по рождению запись остаётся «возможным
        совпадением», хотя речь явно о другом человеке. Паспорт должника выдан
        29.01.2015, а умершим паспорта не выдают.

        Дата выдачи паспорта здесь на тех же правах, что дата рождения, и это
        осознанно: оба поля приходят одним ответом моста по телефону, и
        доверять одному для отбраковки, а второму нет — непоследовательность,
        которая стоит владельцу семи лишних «возможных совпадений» против
        одного.
        """
        known = [value for value in (self.birth_date, self.passport_issued) if value is not None]
