...
class InheritanceCase(SourcedFact):
    ...
    **Реестр ищет только по ФИО и возвращает всех однофамильцев разом.**
    ...
    # Состояние дела, прочитанное из самой записи, а не выведенное из даты.
    # Разные вещи: дату можно не разобрать (реестр пишет их как ГГГГММДД, и
    # общий парсер на таком молча отдаёт ``None``), и тогда закрытое дело
    # объявлялось открытым — а это другой следующий шаг и другие сроки.
    # ``None`` — состояние неизвестно; тогда решает дата, как и раньше.
    case_closed: bool | None = None
    notary_name: str | None = None
    chamber_name: str | None = None
    district_name: str | None = None
    # Сколько ВСЕГО дел реестр вернул по этому ФИО. Живёт в записи, а не только
    # в примечаниях результата, потому что доходить это число обязано до
    # вердикта и до фактора оценки: «дело такое-то» и «дело такое-то из 1730
    # однофамильцев» — разной силы утверждения, а печаталось до сих пор одно и
    # то же. Переживает кэш вместе с записью; ``None`` — у записей, собранных
    # не этим источником или восстановленных до появления поля.
    namesake_count: int | None = None
    # Две и более записи с СОВПАВШЕЙ датой рождения называют разные даты
    # смерти. Это не «мало данных», а доказанная неоднозначность: подтвердить
    # по такому набору смерть должника нельзя. Матчер опускает такие записи до
    # «возможного совпадения» — см. ``IdentityMatcher.assess``.
    ...
    def is_open(self) -> bool:
        ...
        Состояние из записи главнее даты закрытия: неразобранная дата — это
        незнание, а не открытое дело.
        """
        if self.case_closed is not None:
...
class DebtorReport(BaseModel):
    ...
    def fact_count(self) -> int:
        """Сколько НАЙДЕНО о должнике — записей во внешних источниках.
        ...
        **Своя строка из выгрузки сюда не идёт, и это правка по жалобе
        владельца.** Она считалась наравне с находками, поэтому у должника с
        одним производством кнопка писала «2 записи»: производство и мы сами.
        Сложить эти две владелец не мог — карточка прямо над кнопкой печатает
        «Исполнительные производства: 1» и шесть «нет», а наш долг стоит ещё
        выше отдельной строкой «Наш долг: 5 000 ₽». Хуже того, число не
        сравнивалось между проверками: у должника из базы оно было на единицу
        больше, чем у того же человека, проверенного не из базы. Теперь на
        кнопке — только то, что нашли на стороне, и это сходится со строками
        карточки.

        Четыре источника, подключённые последними, считаются здесь наравне с
        остальными: их разделы на странице есть, и кнопка, обещающая её
        содержимое, не вправе о них молчать. Должник в розыске — это самая
        дорогая запись отчёта, и она не может не попасть в счёт.
        """
        return sum(
            len(items)
            for items in (
