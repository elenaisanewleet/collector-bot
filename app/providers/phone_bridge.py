...
def _read_rows(
    ...
    logger.info(
        ...
        **_rejection_report(rejected),
        # ИЗ КАКИХ УТЕЧЕК СОСТОИТ ОТВЕТ. Единственное в этой строке, что можно
        # сверить с проверкой вручную, — и расхождение между ответом боту и
        # ответом владельцу больше ничем не поймать.
...
def _origins(rows: list[RecordDict]) -> list[str]:
    """Из каких утечек состоит ответ — ИМЕНАМИ И ЧИСЛАМИ.

    Имя утечки персональными данными не является, зато это единственное, что
    можно сверить с проверкой вручную: «в моём ответе governmentservices есть,
    а в вашем?»

    Строка завелась на живом расхождении. Боту поставщик дважды прислал
    тринадцать блоков без паспорта, а владельцу на тот же номер — блоки с
    паспортом и с верным адресом. Отбраковка тут ни при чём (``dropped_carried``
    это и показал), значит расходятся сами ответы, и разбирать надо, чем
    отличается запрос. По числу блоков этого не видно, по составу утечек —
    видно сразу.
    """
    counts = Counter(str(_first(row, *_ORIGIN_KEYS) or "—") for row in rows)
