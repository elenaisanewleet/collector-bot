...
class NewDBLegalCasesProvider(NewDBMethodProvider):
    ...
    async def _dispatch(self, subject: SearchSubject, context: FetchContext) -> ProviderResult:
        ...
        results = await asyncio.gather(
            *(self._cases_for(company) for company in checked)
        )
    ...
    async def _cases_for(self, company: BusinessRelation) -> list[LegalEntityCase]:
        ...
        async with self._gate:
            rows, _raw = await self.raw_rows_for(
                NEWDB_METHOD, {"country": COUNTRY_RU, "inn": inn}
            )
    ...
    async def _store(self, inn: str, cases: Sequence[LegalEntityCase]) -> None:
        ...
        payload = json.dumps(
            [case.model_dump(mode="json") for case in cases], ensure_ascii=False
        )
        async with self._database.session() as session:
...
def _cap_notes(
    companies: Sequence[BusinessRelation], *, checked: int, cap: int
) -> tuple[str, ...]:
...
def _role_of(
    entry: Mapping[str, Any], detail: Any, *, inn: str, name: str | None
) -> CourtCaseRole:
