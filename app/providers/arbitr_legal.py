...
def _companies_of(records: Sequence[Any]) -> list[BusinessRelation]:
    ...
    # Порядок — тот, в котором ЕГРЮЛ их вернул. Раньше вперёд выносились
    # компании с ``bankruptcy_flag``, но живой ответ показал этот признак
    # выставленным у должника, по которому ``bankrot_person`` вернул пусто:
    # он означает не то, что кажется, и приоритет по нему был бы приоритетом
    # по шуму. Когда появится проверенный признак, сортировка вернётся сюда.
    return companies
