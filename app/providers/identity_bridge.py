...
*   **Секция ответа выбирается, а не известна.** Документация вендора говорит,
    что ``passport_fns`` отвечает в ``results.company``; живое API отвечает в
    ``results.passport_fns``, по имени метода, как все остальные. Проверено
    прогоном на проде 08.09.2026: конверт содержит
    ``results.passport_fns.result.data[0].innfiz``, а ``results.company`` в нём
    нет вовсе. Пока код верил документации, он поднимал ``unexpected_schema``
    на каждом успешном ответе — вместе с уже полученным ИНН. Обе формы теперь
    рабочие, выбор делает :func:`app.providers.newdb._section_of`.
