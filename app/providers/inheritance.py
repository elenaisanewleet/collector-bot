...
*   запись, чья дата смерти раньше, чем должник ЗАВЕДОМО был жив,
    отбрасывается: это единственный различитель, работающий при пустом
    ``BirthDate``. Дату считает сам субъект (``known_alive_at``) и берёт не
    только рождение — паспорт, выданный после этой смерти, довод такой же
    твёрдый.
...
смерть раньше, чем должник заведомо был жив, — и ответ при этом полный. Второе не тревога, а
...
def _build_result(
    ...
    *,
    ...
    sorted_rows = _partition(answer.rows, subject.birth_date, subject.known_alive_at)
...
def _partition(
    ...
    for row in rows:
        ...
        if birth_date is None or record_birth is None:
            ...
            # смерти: раньше, чем должник заведомо был жив, он умереть не мог.
            #
            # Сверяется с ``known_alive_at``, а не с одним рождением, и та же
            # дата уходит матчеру: счёт «отнесены к другим людям» считается
            # здесь, а таблицу отбирает он, и разойдись они — отчёт сообщил бы
            # «исключено семь», спрятав восьмую запись без объяснения.
            if case.died_before(alive_at or birth_date):
                excluded += 1
            else:
                undated.append(case)
            continue
