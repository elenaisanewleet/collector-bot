...
def _pick_address(rows: list[RecordDict]) -> str | None:
    ...
    ОПОРНОГО БЛОКА ЗДЕСЬ НЕТ, И ЭТО НЕ УПУЩЕНИЕ. В телефонном мосте адрес
    берётся прежде всего из опорного блока, потому что родню там определяет
    правило «не противоречит»: блок с одним адресом и без идентификаторов
    попадает в родню, ничего про себя не доказав. Здесь ``rows`` уже отобраны
    ПОЛОЖИТЕЛЬНЫМ совпадением — фамилия и точная дата рождения
    (:func:`_is_the_same_person`), — то есть каждая запись про нашего человека,
    и выделять среди них опорную незачем. Если фильтр когда-нибудь ослабят до
    «не противоречит», опорный блок понадобится и тут.
    """
