...
class Settings(BaseSettings):
    ...
    # Ссылка живёт ограниченное время: за ней персональные данные должника, а
    # страница открывается без пароля — непредсказуемость адреса и срок жизни
    # здесь и есть контроль доступа.
    #
    # Тридцать дней, а не трое суток, по прямому решению владелицы: «давай уберем
    # срок или сделаем его 30 дней». Прежние семьдесят два часа снимались с
    # мысли «ссылка нужна, пока читают отчёт», а продукт устроен иначе — по
    # ссылке возвращаются: к отчёту перед подачей заявления, к списку базы через
    # неделю, к странице проверок за прошлый месяц.
    #
    # Совсем без срока — нет, и это единственное, где решение владелицы сужено:
    # адрес без срока живёт в пересланных сообщениях вечно, а отозвать его можно
    # только вручную и только вспомнив, что он есть. Тридцать дней — это потолок,
    # а не обязанность: ``/revoke`` гасит ссылку в любой момент, и с месячным
    # сроком это уже не запасной выход, а рабочий инструмент.
    share_link_ttl_hours: Annotated[int, Field(ge=1, le=24 * 30)] = 24 * 30
    # У списков срок был короче в шесть раз: за одной ссылкой стоит вся выгрузка,
    # а не один человек, и радиус поражения отличается на три порядка. Довод
    # верный, но цену платил не тот: двенадцать часов означали, что страница базы
    # и страница проверок протухали к следующему утру, и владелец каждый день
    # начинал с поиска новой ссылки в переписке. Сравнялись — а разница в радиусе
    # теперь живёт там, где ей и место: списки открыты только владельцу.
    share_queue_ttl_hours: Annotated[int, Field(ge=1, le=24 * 30)] = 24 * 30
