...
class Settings(BaseSettings):
    ...
    # ------------------------------------------------- мосты против реестров
    # Уходить ли в ПЛАТНЫЕ реестры сразу после того, как личность собрана по
    # номеру телефона.
    #
    # Раньше это было зашито в «да» дословным требованием владелицы: «ввёл
    # номер — сразу отчёт». Требование она же и развернула, когда продукт дошёл
    # до отладки: «сделаем отдельно по номеру у нас запросы в дипсерч, мы
    # получаем карточку человека — надо это протестировать отдельно, а не идти
    # сразу в NewDB и тратить деньги».
    #
    # Оба требования настоящие и относятся к разным дням. Мост по телефону и
    # реестры — это ДВА поставщика с двумя счетами, и отладка одного не должна
    # оплачивать второй: одна проверка человека с паспортом — пять обращений к
    # NewDB, а проверок личности за вечер делается двадцать.
    #
    # Выключено по умолчанию, потому что ошибка в эту сторону стоит лишнего
    # нажатия, а в обратную — денег. Для показа заказчику ставится ``true``, и
    # сценарий «ввёл номер — сразу отчёт» возвращается целиком.
    auto_check_after_lookup: bool = False
    ...
    # ---------------------------------------------------------------- прогон по всей базе
    # Одно нажатие тратит деньги за ВСЮ выгрузку: две тысячи должников по шесть
    # обращений — это двенадцать тысяч платных запросов. Кнопка при этом стояла
    # в главном меню владельца рядом с обычной проверкой, а владелец у продукта
    # не технический.
    #
    # Поэтому по умолчанию выключено, и выключено настройкой, а не удалением:
    # прогон — рабочая часть продукта, он понадобится, когда будет куплен тариф.
    # Пока его нет, кнопки нет в меню, команды нет в синем списке, а /batch
    # честно отвечает, что выключен, — вместо того чтобы молча ничего не делать.
    ...
    # СРОКА ЖИЗНИ У ССЫЛКИ НЕТ. Ноль значит «бессрочно», и это умолчание.
    #
    # Сроки были 72 часа у отчёта и 12 часов у списков. Оба снимались с мысли
    # «ссылка нужна, пока читают отчёт», и оба оказались неверны, потому что
    # продукт устроен иначе: по ссылке ВОЗВРАЩАЮТСЯ — к отчёту перед подачей
    # заявления, к списку базы через месяц, к результату прогона через полгода.
    #
    # Решающий довод владелицы про деньги: «владелец заплатит 30 к, прогонит всю
    # базу, а у него через 30 дней не будет результата». Прогон по двум тысячам
    # должников стоит около тридцати тысяч рублей, и срок жизни ссылки на его
    # результат — это срок, после которого за те же данные надо платить снова.
    # Никакой довод о приватности столько не стоит.
    #
    # Чем держится доступ, если не сроком. Токеном: 256 бит из ``secrets``, то
    # есть адрес, который не подбирают и не перебирают. Срок жизни никогда и не
    # был здесь основной защитой — он был страховкой от утечки самого адреса, и
    # эту роль полностью берёт на себя ``/revoke``: одна команда гасит ссылку
    # или все сразу, немедленно и навсегда.
    #
    # Настройка остаётся: развёртывание, которому срок нужен, ставит часы. Год —
    # потолок значения, а не срок по умолчанию.
    share_link_ttl_hours: Annotated[int, Field(ge=0, le=24 * 365)] = 0
    # Списки — база, очередь, проверки не из базы. Держались двенадцать часов:
    # за одной такой ссылкой стоит вся выгрузка, а не один человек, и радиус
    # поражения отличается на три порядка. Довод верный, но цену платил не тот:
    # страница базы протухала к следующему утру, и владелец каждый день начинал
    # с поиска новой ссылки в переписке. Разница в радиусе живёт теперь там, где
    # ей и место: списки выдаются только владельцу.
    share_queue_ttl_hours: Annotated[int, Field(ge=0, le=24 * 365)] = 0
