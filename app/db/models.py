...
class Debtor(Base):
    ...
    # ИНН физлица из выгрузки. Без него три источника — ЕГРИП со связями по
    # юрлицам, банкротство и арбитраж физлица — не запрашиваются вовсе: они
    # ищут только по innfiz.
