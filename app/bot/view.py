...
def report_card(
    ...
    *,
    ...
    # Карточка — выжимка, и она обязана сказать, что за кнопкой лежит остальное.
    # Иначе оператор читает пять строк и считает, что это всё, что известно о
    # человеке. Раньше здесь перечислялись источники — производства, банкротство,
    # залоги, суды, связи с юрлицами; перечисление убрано. Заказчику нужны номер
    # телефона и сводка, а не список того, что у нас под капотом; чем именно
    # собрано, видно на самой странице, и там этому место.
    lines.append("")
