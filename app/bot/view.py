...
def report_card(
    ...
    *,
    ...
    # «Это прошлая проверка» — ДО вердикта, а не после списка источников.
    # Утверждение о том, СВЕЖИЕ ли данные, относится ко всему, что ниже, и
    # обязано стоять до них, как баннер демо-режима.
    cache = _cache_line(report)
    if cache:
        lines.extend(("", *cache))
    ...
    # Строка про кэш переехала ВВЕРХ (см. ``_cache_line``) и здесь больше не
    # печатается: внизу она стояла последней, мелким серым, ниже списка
    # источников — и трижды подряд была прочитана как часть подписи.
...
CACHED_LEAD = "Это прошлая проверка от {when}"
CACHED_ACTION = "Источники сейчас не опрашивались. «Спросить заново» — опросить их и заплатить."


def _cache_line(report: DebtorReport) -> list[str]:
    """«Это прошлая проверка» — громко, сверху и с названием кнопки.
    ...
    честно говорила. Строкой «Данные проверки от 16.09.2026 15:23»: последней,
    обычным текстом, ниже списка источников и ниже блока пропусков. Там её
    читают как подпись под документом, а не как предупреждение.

    Дефект был не в правдивости, а в весе. Утверждение «всё, что выше, снято
    вчера» относится ко всей карточке и обязано стоять ДО неё — там же, где
    стоит баннер демо-режима, и по той же причине.

    ВТОРАЯ СТРОКА НАЗЫВАЕТ ДЕЙСТВИЕ. Без неё сообщение сообщало о проблеме и
    молчало о выходе: оператор, поняв, что данные несвежие, шёл вводить номер
    заново — то есть делал единственное, что кэш обойти НЕ МОЖЕТ (вопрос тот
    же, ответ из кэша). Кнопка названа по имени, потому что кнопок под
    отчётом пять.
    """
    if not report.from_cache:
        return []
    return [
        bold(esc(CACHED_LEAD.format(when=format_datetime(report.cached_at)))),
        esc(CACHED_ACTION),
    ]
