...
def main_reply_keyboard(*, owner: bool) -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        ...
        is_persistent=True,
        # Ровно то же, что просит первый шаг («✎ Телефон»), — иначе поле ввода
        # и экран зовут разное, и оператор выбирает, кому верить. Перечисление
        # «госномер, телефон, ИНН» стояло здесь потому, что телефона в выгрузке
        # нет ни у одного должника; выбор владелицы это не отменило, но платит
        # за него код, а не текст: любой номер шаг по-прежнему принимает молча.
        # Поле и экран первого шага зовут одно и то же: расхождение заставляет
        # выбирать, кому верить. Обе двери принимают номер и ФИО.
        input_field_placeholder="Номер телефона или ФИО должника",
    )
