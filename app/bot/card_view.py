...
#: Строка под «✎ Телефон». Просит номер и на этом кончается: перечисление
#: «госномер, телефон, ИНН или номер договора» владелица прислала скриншотом
#: как то, что надо убрать. Слово «номер», а не «телефон», — единственная
#: оставленная поблажка: шаг и правда принимает любой номер, и строка, обещающая
#: строго телефон, врала бы тому, кто прислал госномер и получил отчёт.
ASK_PROMPT_NUMBER = "Отправьте номер сообщением."
...
#: человека. Паспорт это персональные данные, и то, что бот их не хранит, надо
#: сказать ДО ввода, а не в справке. Остальным полям такой строки не положено:
#: приписка к каждому вопросу — снова полотно.
ASK_NOTES: dict[str, str] = {
    Field.PASSPORT.value: "Номер не сохраняю и сообщение удалю.",
    # У телефона оговорки нет, хотя первый шаг теперь просит именно его. Дело
    # не в том, что она неверна: полный номер живёт только в памяти процесса, в
    # карточку едет маска. Дело в длине — экран первого шага это четыре строки,
    # и пятую владелица возвращала. Вдобавок шаг принимает любой номер, а
    # «номер не сохраняю» про госномер — просто неправда.
}
...
HINTS: dict[str, str] = {
    ...
    Field.PHONE.value: (
        "Телефон. Поднимает вашу запись о должнике. Номер не сохраняю, только маску."
    ),
...
PASSPORT_HINT_STORED = (
    f"Серия и номер паспорта, {PASSPORT_LENGTH} цифр. Сам по себе не открывает ничего — "
    "по нему ФНС выдаёт ИНН, и тогда откроются те же три источника. Запрос платный. "
    "Ваше сообщение с номером я удалю, но сам номер сохранится в базе: так "
    "настроен этот бот."
)
