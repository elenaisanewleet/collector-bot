...
#: Присланное ничего не изменило — то же значение второй раз или строка, из
#: которой нечего взять. Говорится вслух, потому что альтернатива — либо
#: молчание (заказчик читает его как «бот сломался»), либо переезд карточки
#: вниз новым сообщением (он же читает это как «бот шлёт одно и то же»).
NOTHING_NEW = "Это у меня уже записано — ничего не изменилось."
