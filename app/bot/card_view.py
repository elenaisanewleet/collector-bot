...
QC_RESET = f"{QC}:reset"
#: Спросить поставщика по номеру телефона — отдельным нажатием, без реестров.
QC_LOOKUP = f"{QC}:lookup"
...
#: Спросить поставщика по номеру — ОТДЕЛЬНЫМ нажатием. Дословная просьба
#: владельца: «надо добавить тут функционал — собрать данные по номеру».
#:
#: До этого мост звался сам и только один раз: когда в карточке нет имени.
#: Переспросить было нечем ни после перезапуска (номер живёт в памяти
#: процесса), ни после «Сбросить данные», ни просто так — оператор видел
#: карточку и не мог сказать «сходи по номеру ещё раз».
LOOKUP_LABEL = "Собрать данные по номеру"
#: Что именно приехало — ИМЕНАМИ ПОЛЕЙ. Значения оператор видит в самой
#: карточке строкой выше, а вот «сходил и ничего нового» от «сходил и заполнил
#: паспорт» без этой строки не отличается ничем.
LOOKUP_FILLED = "Собрал по номеру: {fields}."
LOOKUP_NOTHING_NEW = (
    "По номеру ничего нового — всё, что нашлось, в карточке уже есть. "
    "Чтобы перезабрать, сначала «Сбросить данные по этому человеку»."
)
#: Номер живёт в памяти процесса и не переживает перезапуск: в базе от него
#: остаётся маска. Кнопка при этом не прячется — она объясняет, чего не хватает.
LOOKUP_NEEDS_PHONE = "Для этого нужен номер. Нажмите «Телефон» и пришлите его."
...
#: «Ну не по умолчанию — а по логике, алгоритму: это наибольшая вероятность
#: фактического места проживания.» Поправка владельца, и она по делу: «первый
#: подставлен по умолчанию» читается как «взят первый попавшийся», хотя за
#: выбором стоит его же правило — сперва самая доверенная утечка, где назван
#: этот человек, и только потом порядок и частота.
#:
#: Вторая половина строки осталась: вероятность — не факт, и обещать обратное
#: нельзя. Выбор всё равно за оператором, он знает своего должника.
#:
#: СЛОВА «УТЕЧКА» НА ЭКРАНЕ НЕТ, и это не эвфемизм ради красоты. Внутри разбора
#: оно точное — блоки ответа и правда происходят из разных баз, — но на экране
#: читается как признание, откуда бот берёт данные, и попадает в чужие руки
#: вместе со скриншотом. Снаружи у нас один словарь на весь продукт: источник.
ADDR_LEAD = (
    "Первый выбран алгоритмом: адрес из самого надёжного источника, где назван "
    "этот человек. Это наибольшая вероятность фактического проживания, а не "
    "подтверждённый факт."
)
...
#: То же самое, когда личность УЖЕ собрана мостом. «Не нашёл» рядом с
#: заполненной карточкой читается как противоречие — человек-то вот он. Новость
#: здесь другая и она про дело: этого должника у вас ещё не заведено.
NOT_IN_EXPORT_NEW = "В вашей базе такого должника нет — это новый клиент."
...
def _text(
    ...
    *,
    ...
    if notice:
        lines.extend(("", notice))
    # Ниже — только то, что отвечает на вопрос «что делать прямо сейчас».
    #
    # Отсюда убраны три блока, и убраны по дословной жалобе владелицы: «в
    # выдаче мне не нравится текст лишний… вообще непонятно о чём он».
    #
    # * «Прочерк — это „я не спрашивал“» объясняло прочерки на карточке, где их
    #   почти не осталось: личность приезжает по номеру заполненной.
    # * «Сейчас спрошу: 6 источников из 9 / Не спрошу: 3 — нужен ИНН физлица»
    #   считало источники ДО проверки. Число без имён ни о чём не говорит тому,
    #   кто не знает, что за источники, а после автопрогона оно и вовсе не
    #   успевает пригодиться.
    # * «Пришлите сообщением — допишу сюда же» повторяло то, что и так написано
    #   в подсказке поля ввода.
    #
    # Инвариант «не спрашивали ≠ не найдено» при этом никуда не делся: он живёт
    # в ОТЧЁТЕ (``run_notes``), где относится к уже потраченным деньгам и где
    # его читают по делу. Карточка — это форма ввода, а не место для оговорок.
...
#: Как назвать в ответе то, что заполнил мост, и в каком порядке перечислять.
#: Имя приезжает одним полем: в карточке это три строки, но находка — одна, и
#: «Фамилия, Имя, Отчество» читалось бы как три разных.
_COLLECTED_TITLES: dict[str, str] = {
    "name": "ФИО",
    "birth_date": FIELD_TITLES["birth_date"],
    "inn": FIELD_TITLES["inn"],
    "passport": FIELD_TITLES["passport"],
    "passport_issued": FIELD_TITLES["passport_issued"],
    "snils": FIELD_TITLES["snils"],
    "address": FIELD_TITLES["address"],
}


def collected_notice(filled: frozenset[str]) -> str:
    """Что приехало по номеру — именами полей, или честное «ничего нового».

    Значения оператор видит в карточке строкой выше, а вот «сходил и ничего не
    нашёл» от «сходил и заполнил паспорт» без этой строки не отличается ничем:
    экран в обоих случаях один и тот же.
    """
    named = [title for name, title in _COLLECTED_TITLES.items() if name in filled]
    return LOOKUP_FILLED.format(fields=", ".join(named)) if named else LOOKUP_NOTHING_NEW
...
def _empty_card_hint(card: Card) -> str | None:
    """Что нужно, чтобы вообще было чем проверять. Только на пустой карточке.

    Это единственное, что осталось от прежнего блока покрытия: на заполненной
    карточке подсказка не нужна, а на пустой она отвечает на настоящий вопрос —
    «а что от меня хотят».
    """
    if card.subject() is not None:
        return None
    return "Нужны фамилия с именем — или ИНН, или госномер. Остальное дописывается кнопками ниже."
...
def _run_row(
    ...
    return [
        [_button(run, QC_RUN)],
        # «Собрать данные по номеру» — сразу под «Проверить», потому что это
        # порядок работы: сперва по номеру собирается личность, потом по ней
        # спрашиваются реестры. Кнопка появляется, как только номер в карточке
        # назван, — в том числе когда от него осталась одна маска: тогда она
        # объясняет, что номер нужно прислать заново, а не молчит.
        *([[_button(LOOKUP_LABEL, QC_LOOKUP)]] if card.phone or card.phone_masked else []),
