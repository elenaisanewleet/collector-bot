...
#: Первый шаг спрашивает не телефон, а любой номер, и говорит об этом одной
#: строкой вместо примера с подписью. Телефона в выгрузке нет ни у кого, а
#: госномер есть почти у всех — если не сказать, оператор пришлёт телефон и не
#: найдёт никого.
ASK_PROMPT_NUMBER = "Госномер, телефон, ИНН или номер договора."
...
ASK_NOTES: dict[str, str] = {
    ...
    # У телефона такой оговорки больше нет: первый шаг принимает ЛЮБОЙ номер —
    # госномер, телефон, ИНН, договор, — и «номер не сохраняю» про госномер
    # просто неправда. Про телефон это по-прежнему верно и по-прежнему сказано,
    # но там, где он и правда телефон: в подсказке поля.
}
