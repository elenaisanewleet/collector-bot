...
точечные линейки из макета («Фамилия ····· Иванова») не выровнялись бы, а
...
#: ВЫМЫШЛЕННЫЕ. До 09.09.2026 здесь стояли настоящие фамилия, имя, отчество и
#: дата рождения живого человека — владелицы, снятые с её же проверки. Их видел
#: на экране каждый, кто открывал бота, и лежали они в публичном репозитории.
#: Пример в интерфейсе обязан быть выдуманным: он показывается всем и навсегда.
ASK_EXAMPLES: dict[str, str] = {
    ...
    Field.LAST_NAME.value: "Иванова",
    Field.FIRST_NAME.value: "Мария",
    Field.MIDDLE_NAME.value: "Сергеевна",
    Field.FIO.value: "Иванова Мария Сергеевна",
    Field.BIRTH_DATE.value: "05.07.1985",
...
#: По номеру имя не определилось — и ТРИ РАЗНЫЕ ПРИЧИНЫ этого названы разными
#: словами. Раньше здесь стояла одна фраза на все случаи, с прямо записанным
#: доводом: «его молчание и его незнание для оператора одно и то же действие».
#: Довод был верен, пока поиск по номеру был удобством. Он перестал быть верным,
#: когда номер стал ГЛАВНЫМ входом продукта, и вот чем это стоило:
#:
#: * упавший источник и честное «никого нет» неразличимы, и владелец идёт
#:   набирать фамилию руками там, где хватило бы повтора через минуту;
#: * ненастроенный мост выглядит как пустой ответ, и на проде это не
#:   диагностируется вовсе — ровно на этом и потерялся день.
#:
#: Разница «спросили и не нашли» против «не спрашивали» — главный инвариант
#: проекта, и держится он теперь и здесь.
NAME_NOT_FOUND = "По номеру никого не нашёл. Введите фамилию."
NAME_SOURCE_SILENT = "Источник по номеру не ответил. Повторите или введите фамилию."
NAME_LOOKUP_OFF = "Поиск по номеру не настроен. Введите фамилию."
