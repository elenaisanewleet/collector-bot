...
#: Показывается, пока идёт проверка, и только когда правда: часть источников
#: ищет только по ИНН физлица. Сказать об этом до отчёта ничего не стоит и
#: избавляет от вопроса «почему тут пусто».
#
# Источники не перечислены в самой строке, и это правка по факту: «банкротство,
# ИП и арбитраж» было верно при трёх таких источниках, их стало шесть, и строка
# стала врать молча. Список приходит из реестра — см. ``Registry.inn_only``.
#
...
NO_INN_NOTE_TEMPLATE = (
    "Без ИНН не спрошу {sources} — добавить можно кнопкой «Уточнить данные» под отчётом."
)
...
async def run_and_send_report(
    ...
    *,
    ...
    #
    # А ВОПРОС ХРАНИТСЯ РЯДОМ, и нужен он ровно «Спросить заново». Кнопка
    # переспрашивала источники, но не личность: она повторяла прогон с уже
    # обогащённым субъектом, а мост по телефону зовётся только когда имени нет.
    # Имя там было — мост пропускался, и однажды неверно выбранный адрес уезжал
    # в Росреестр снова, за деньги и без всякой возможности переспросить.
    token = container.subject_store.put(report.subject, question=subject)
...
def _progress_note(subject: SearchSubject, container: Container) -> str | None:
    ...
    писалась ДО поиска и по субъекту, каким он был до него, — а ИНН добывается
    ВНУТРИ поиска, мостом «паспорт → ИНН». С паспортом на руках предсказание
    оказывалось ложным ровно тогда, когда всё получалось: бот обещал не спросить
    источники и тут же их спрашивал.
    ...
    Сами источники перечисляет реестр, а не эта строка: их было три, стало
    шесть, и вписанный руками список успел устареть молча.
    """
    ...
    if bridge is not None and bridge.will_query(subject):
        return None
    sources = view.inn_only_sources(container.registry.inn_only)
    if not sources:
        # Ни один источник по ИНН не подключён — терять нечего, и обещать
        # оператору, что он что-то упустит, не за что.
        return None
    return NO_INN_NOTE_TEMPLATE.format(sources=sources)
