...
async def show(
    ...
    *,
    ...
    if _unchanged(container, card, screen):
        if not answering:
            return
        # Экран не изменился, а человек написал — ответить обязаны. Раньше здесь
        # карточка снималась и уезжала вниз новым сообщением, и заказчик увидел
        # то, на что и пожаловался: «несколько раз пришло одно сообщение, хотя
        # надо чтобы просто обновлялось одно».
        #
        # Поэтому вместо переезда — оговорка. Она меняет текст, значит правка на
        # месте пройдёт (Telegram отвергает только ОДИНАКОВУЮ), и карточка
        # остаётся одна. И она честнее переезда: говорит, почему ничего не
        # изменилось, вместо того чтобы молча мигнуть тем же экраном.
        screen = card_view.screen(
            card,
            registry=container.registry,
            store_sensitive=container.settings.store_sensitive_identifiers,
            notice=notice or card_view.NOTHING_NEW,
            conflict=conflict,
        )
        if _unchanged(container, card, screen):
            # Оговорка уже висела — второй раз то же самое, и правка вернула бы
            # 400. Тогда карточка всё-таки переезжает вниз: это редкий случай
            # (одно и то же прислали трижды), и молчание тут хуже переезда.
            await _drop_card_message(message, container, card)
