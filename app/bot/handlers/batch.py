...
#: Прогон выключен настройкой. Текст называет причину и цену, а не «недоступно»:
#: выключатель поставлен по прямой просьбе владелицы — «уберём кнопку проверить
#: всю базу, чтобы владелец не нажал её и не ушёл в какой-то лимит», — и человек,
#: который до этого экрана всё-таки дошёл, имеет право знать, что именно
#: выключено и чем это включается.
BATCH_OFF = (
    "Проверка всей базы выключена.\n\n"
    "Одно нажатие опрашивает платные источники по каждому должнику выгрузки, "
    "и включать это стоит осознанно — когда оплачен тариф под нужный объём.\n\n"
    "Включается на сервере: BATCH_ENABLED=true в .env и перезапуск.\n"
    "Проверка по одному человеку работает как обычно."
)
...
def render_estimate(estimate: BatchEstimate) -> str:
    ...
    if estimate.cached:
        # Строка «Уже проверено недавно: 0» — не информация, а шум: на первом
        # прогоне она стоит в смете всегда и всегда с нулём.
        lines.append(
            f"Уже проверено недавно: {estimate.cached}, их спросим только заново, если "
            "в прошлый раз ответили не все источники"
        )
    lines.append(f"Нужно опросить: {estimate.to_query}")
    lines.extend(_cost_lines(estimate))
    ...
    if estimate.capped:
        lines.append("")
        lines.append(
            f"За один раз проверяю не больше {estimate.debtors}: остальные "
            "останутся непроверенными, запустите второй прогон."
        )
    lines.append("")
    lines.append(
...
def _cost_lines(estimate: BatchEstimate) -> list[str]:
    ...
    if cost is None:
        ...
        # Ноль рублей означал бы «бесплатно». Правда — «цена неизвестна», и
        # сказать это надо словами. Имени переменной окружения здесь нет: смету
        # читает оператор, а не тот, кто правит .env, — ему это ничего не даёт.
        lines.append("Во сколько это встанет в рублях — сказать нечем: цена обращения не задана.")
    else:
        lines.append(
...
def _bridge_lines(estimate: BatchEstimate) -> list[str]:
    ...
    if estimate.without_inn > 0:
        # Короче прежнего на две строки, но предупреждение остаётся: пустой
        # раздел по этим должникам будет значить «не спрашивали», а не «чисто»,
        # и узнать это оператор обязан до того, как заплатит.
        return [
            f"ИНН неизвестен у {estimate.without_inn}: часть проверок по ним не пройдёт, "
            "и пустой раздел у них будет значить «не спрашивали»."
        ]
    return []
...
async def offer_batch(
    ...
    Здесь же единственная проверка выключателя, и стоит она здесь именно потому,
    что все три двери — команда, инлайн-кнопка и кнопка нижней клавиатуры —
    сходятся в этой функции. Прятать кнопку в меню недостаточно: нижняя
    клавиатура живёт на стороне Telegram и переживает и обновление бота, и смену
    настроек, а ``/batch`` можно набрать руками.
    """
    if not container.settings.batch_enabled:
        await state.clear()
        await message.answer(BATCH_OFF, reply_markup=main_menu(owner=True))
        return
